...
const DEMO_ACCOUNTS = [
  { label: "관리자", email: "admin@bandstage.dev", password: "admin1234!", color: "#dc2626" },
  { label: "아티스트", email: "artist@bandstage.dev", password: "artist1234!", color: "#0d28c4" },
  { label: "공연장", email: "venue@bandstage.dev", password: "venue1234!", color: "#059669" },
];
    ...
    <div className="flex min-h-[calc(100vh-200px)] items-center justify-center py-12">
      <div className="w-full max-w-md space-y-8">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-gray-600">Band-Stage에 오신 것을 환영합니다</p>
        </div>
        ...
        <div className="rounded-xl border border-dashed border-gray-300 bg-gray-50 p-5">
          <p className="text-xs font-semibold text-gray-500 mb-3 text-center">데모 계정으로 빠른 로그인</p>
          <div className="flex flex-col gap-2">
            {DEMO_ACCOUNTS.map((account) => (
              <button
                ...
                className="flex items-center justify-between rounded-lg border border-gray-200 bg-white px-4 py-2.5 text-sm hover:border-gray-300 transition-colors disabled:opacity-50"
              >
                <span className="flex items-center gap-2">
                  <span className="h-2 w-2 rounded-full" style={{ backgroundColor: account.color }} />
                  <span className="font-medium text-[#0b1021]">{account.label}</span>
                  <span className="text-gray-400 text-xs">{account.email}</span>
                </span>
                <span className="text-xs text-gray-400">클릭하여 로그인 →</span>
              </button>
            ))}
          </div>
          <p className="text-[11px] text-gray-400 mt-3 text-center">
            DB 없이도 동작하는 테스트 계정입니다
          </p>
        </div>
      </div>
    </div>
